...
* Das Modell hat **13 freie Parameter** ($\lambda_{21}$, $\lambda_{63}$, $\beta_{31}$, $\beta_{32}$, $\psi_{11}$, $\psi_{22}$, $\psi_{33}$, $\theta_1$, $\theta_2$, $\theta_3$, $\theta_4$, $\theta_5$, $\theta_6$) bei 21 nicht-redundanten Elementen in $\boldsymbol{\Sigma}$ (6 Varianzen + 15 Kovarianzen = 21 = 6 * 7 / 2, d.h. $df = 21 - 13 = 8$ Freiheitsgrade.
